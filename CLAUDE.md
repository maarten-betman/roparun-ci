...
- The frontend is one pnpm/npm workspace with two Vite inputs (`vite.config.ts` → `rollupOptions.input`). Shared code lives under `src/map/` (style resolver), `src/api/` (typed client + DTOs), `src/chrome/` (shared TopBar + logo assets), and `src/styles/tokens.css` (design-system CSS custom properties — imported once in `src/viewer/main.tsx`). Feature code is siloed under `src/viewer/`, `src/planner/`, `src/tracker/`. The tracker entry is the PWA — keep its bundle small (no MapLibre unless strictly needed). `src/viewer/main.tsx` is the single entry for `index.html` and dispatches by pathname: `/planner*` renders the planner, `/t/:slug/:year` renders the public viewer for that team+year, everything else renders the default viewer.
- Design chrome: a 48 px navy `TopBar` (Conclusion Intelligence logo chip, title, year meta, V-concept pill, Share + Download GPX actions) sits on top of both viewer and planner; map and sidebar offset by `var(--topbar-height)`. Static brand logos live at `frontend/public/assets/conclusion-intelligence-logo.png` and `roparun-logo.webp`, served at `/assets/...`. All component CSS reads from `tokens.css`; don't hard-code colours or spacing.
...
- Viewer rendering: `frontend/src/viewer/catalog.ts` is the single source of truth for layer/category display names, colors, default-visible flag, render style (`marker` vs `trace`), and per-role view presets. Colors match the official Roparun scheme: yellow runners, blue A/B-vehicle overlays, cyan C-vehicle, red forbidden. `Viewer.tsx` builds GeoJSON FCs once and uses MapLibre `setFilter` to toggle visibility (no re-render on toggle). Heavy point clouds (vehicle_*_allowed/forbidden, runner_route_points, km_markers) default to **off**; checkpoints/handovers/water-stops/hazards/sleeping/toilets/passages default to **on**. Vehicle road overlays render as small unstroked dots (`style: trace`) so the dense per-meter point clouds visually read as a coloured line along the road, matching the Garmin/OsmAnd map look. Sidebar UI lives in `Sidebar.tsx` + `sidebar.css`: a "View as" row of role presets (Lopers/A/B/C-voertuig) replaces the visibility sets in one click; on screens <=720 px wide the sidebar collapses to a bottom drawer toggled by the `Layers` button. The route name is matched against `V[1-3]` to display a "concept — wait for V4 before the event" warning in the header (per the organizer doc, V4 is published the Friday before the event).
...
- Live positions flow: **tracker PWA** → batch `POST /ingest` (bearer-token auth, Phase 3 v1 uses a localStorage queue; IndexedDB + Background Sync deferred to 3.5) → `position` table (BRIN on `ts`, GIST on `geom`) → in-memory fan-out via `services/live_hub.py` → WebSocket `/ws/live/{team_slug}/{year}` → **viewer** renders coloured marker + breadcrumb per device. REST fallback for initial snapshot: `GET /public/{slug}/{year}/live`. One uvicorn worker only — multi-worker deploys would need Redis pub/sub (deferred). Simulator: `make simulate gpx=<path> token=<bearer>` replays a GPX against `/ingest`.
- Device pairing: the planner mints a short-lived **pairing token** (`POST /pairing-tokens`, 30-minute TTL, role baked in) which the `PairingPanel` renders as a QR code + copyable `tracker.html?pair=<token>` URL. The tracker entry strips the `?pair=` param via `history.replaceState`, asks only for a name, and redeems it at `POST /devices/from-pairing` — a single transaction that mints the persistent `Device` + bearer token and stamps `redeemed_at`. Tokens are single-use: replay → 409; unknown → 404; expired → 410. The older `POST /devices` endpoint (name + role + team_slug + year) is still the fallback for hand-typed pairing. Model + migration: `pairing_token.py` / `0006_pairing_tokens`. Role enum is reused from `0004` via `postgresql.ENUM(..., create_type=False)` — never switch back to `sa.Enum(create_type=False)`, that form silently re-emits `CREATE TYPE` and breaks `alembic upgrade`.

## Working in this repo

- **Do not invent features beyond the current phase.** Check `/root/.claude/plans/i-want-to-create-magical-book.md` for what's in scope now. Phase 0 shipped (scaffold + health probes + placeholder map). Phase 1 shipped (routes CRUD, GPX I/O, planner UI, public viewer endpoint, seed script). Phase 2 shipped (sidebar with layer/category toggles, mobile bottom-sheet, hover popups, default-hidden noisy POI clouds). Phase 3 shipped (live tracking: `/ingest`, WS fan-out, viewer breadcrumbs, GPX simulator). Phase 3.5 partially shipped (QR pairing); still to land: magic-link auth + IndexedDB/Background-Sync PWA queue. Elevation profile is intentionally deferred — source GPX has no `<ele>` data, so it would need an external elevation source.
